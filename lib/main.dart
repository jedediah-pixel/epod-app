...
const String kSpDeviceId    = 'deviceId';
const String kSpDeviceToken = 'deviceToken';
...
class UploadQueue {
  ...
  static const int kMaxAttempts = 32; // hard cap on retries
...
if (idx >= 0) _jobs.removeAt(idx);
  ...
  Duration _delay(int attempts) {
    ...
    // === Device-auth helpers (fallback when no JWT) ===
  Future<void> _ensureDeviceIdentity() async {
    final sp = await SharedPreferences.getInstance();
    var id  = sp.getString('deviceId');
    var tok = sp.getString('deviceToken');
    if (id != null && tok != null && id.isNotEmpty && tok.isNotEmpty) return;

    final r = await http.post(
      Uri.parse('$kApiBase/device/register'),
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({'deviceId': _genDeviceId()}),
    );
    if (r.statusCode != 200) throw PermanentFail('device_register:${r.statusCode}');
    final t = (jsonDecode(r.body)['deviceToken'] as String?) ?? '';
    if (t.isEmpty) throw PermanentFail('device_register:empty_token');

    final body = jsonDecode(r.body);
    await sp.setString('deviceId', body['deviceId'] ?? '');
    await sp.setString('deviceToken', t);
  }

  Future<Map<String, String>> _deviceHeaders(String contentType, String key) async {
    final sp = await SharedPreferences.getInstance();
    var id  = sp.getString('deviceId');
    var tok = sp.getString('deviceToken');
    if (id == null || tok == null || id.isEmpty || tok.isEmpty) {
      await _ensureDeviceIdentity();
      id  = sp.getString('deviceId');
      tok = sp.getString('deviceToken');
    }
    return {
      'x-device-id': id!,
      'x-device-token': tok!,
  ...
  bool _isPermanentDeviceSign(int code, String body) {
...
Future<bool> _process(UploadJob j) async {
  ...
  try {
    ...
    Future<void> _ensureDeviceIdentity() async {
      final sp = await SharedPreferences.getInstance();
      var id  = sp.getString(kSpDeviceId);
      var tok = sp.getString(kSpDeviceToken);
      ...
      if (res.statusCode != 200) {
        throw PermanentFail('device_register:${res.statusCode}');
      }
      tok = (jsonDecode(res.body)['deviceToken'] as String?) ?? '';
      if (tok.isEmpty) throw PermanentFail('device_register:empty_token');

      await sp.setString(kSpDeviceId, id);
      await sp.setString(kSpDeviceToken, tok);
    }
    ...
    Future<Map<String, String>> _deviceHeaders(String contentType, String key) async {
      ...
      if (id == null || tok == null || id.isEmpty || tok.isEmpty) {
        await _ensureDeviceIdentity();
        id  = sp.getString(kSpDeviceId);
        tok = sp.getString(kSpDeviceToken);
      }
      return {
        'x-device-id': id!,
        'x-device-token': tok!,
    ...
    // Treat invalid device credentials as permanent; infra/network as retryable.
    bool _isPermanentDeviceSign(int code, String body) {
      if (code == 401 || code == 403 || code == 404 || code == 400) return true;
      return false;
    }
...
// 1) sign (get presigned PUT URL) — try JWT first, fallback to device auth
    ...
    if (jwtToken != null && jwtToken.isNotEmpty) {
      ...
      if (signRes.statusCode == 200) {
        signed = (jsonDecode(signRes.body)['url'] as String);
      } else {
        // If JWT is invalid for today/day mismatch/etc → fallback to device
        final b = bodyStr.toLowerCase();
        final jwtHardGate =
            signRes.statusCode == 400 || signRes.statusCode == 401 || signRes.statusCode == 403 ||
            b.contains('driver_or_day_mismatch') ||
            b.contains('session_required_for_today') ||
            b.contains('no_session_for_day');

        if (!jwtHardGate) {
          // Likely transient (5xx/network) → let WM retry
          return false;
        }
    ...
    // Device fallback (or primary if no JWT)
    if (signed == null) {
      await _ensureDeviceIdentity();
      final devHdrs = await _deviceHeaders('application/json', key);
      final dRes = await http.post(
        Uri.parse('$kApiBase/sign_device'),
        headers: devHdrs,
        body: jsonEncode({'filename': key, 'contentType': 'image/jpeg'}),
      );
      final db = dRes.body;
      final dlog = db.length > 300 ? '${db.substring(0, 300)}…' : db;

      if (dRes.statusCode != 200) {
        if (_isPermanentDeviceSign(dRes.statusCode, db)) {
          throw PermanentFail('sign_device:${dRes.statusCode}:$dlog');
        }
        return false;
      }
      usedDeviceAuth = true;
      signed = (jsonDecode(dRes.body)['url'] as String);
    }
...
urls.add(_publicUrlFromSigned(signed));
    ...
    if (jwtToken != null && jwtToken.isNotEmpty) {
      ...
      if (manifestSign.statusCode == 200) {
        manifestUrl = (jsonDecode(manifestSign.body)['url'] as String);
      } else {
        final b = mBodyStr.toLowerCase();
        final jwtHardGate =
          manifestSign.statusCode == 400 || manifestSign.statusCode == 401 || manifestSign.statusCode == 403 ||
          b.contains('driver_or_day_mismatch') || b.contains('session_required_for_today') || b.contains('no_session_for_day');
        if (!jwtHardGate) {
          // transient -> let the queue retry
          return false;
        }
        // else fall through to device
      }
    }
    ...
    if (manifestUrl == null) {
      await _ensureDeviceIdentity();
      final devHdrs = await _deviceHeaders('application/json', manifestKey);
      final mDev = await http.post(
        Uri.parse('$kApiBase/sign_device'),
        headers: devHdrs,
        body: jsonEncode({'filename': manifestKey, 'contentType': 'application/json'}),
      );
      final db = mDev.body;
      final dlog = db.length > 300 ? '${db.substring(0, 300)}…' : db;
      if (mDev.statusCode != 200) {
        if (_isPermanentDeviceSign(mDev.statusCode, db)) {
          throw PermanentFail('sign_manifest_device:${mDev.statusCode}:$dlog');
        }
        return false;
      }
      usedDeviceAuth = true;
      manifestUrl = (jsonDecode(mDev.body)['url'] as String);
    }
...
// Normalize common pieces
final uLower = (j.username ?? '').toLowerCase();
final anyKey = 'pods/$day/$uLower/${j.podNo}_1.jpg';

// Pick base headers depending on auth path
final baseHeaders = usedDeviceAuth
    ? await _deviceHeaders('application/json', anyKey)
    : _auth('application/json', anyKey);

// ACK headers = base + x-driver
final Map<String, String> ackHeaders = Map<String, String>.from(baseHeaders);
ackHeaders['x-driver'] = uLower;


// ACK (best-effort)
try {
  await http.post(
    Uri.parse(usedDeviceAuth ? '$kApiBase/ack_device' : '$kApiBase/ack'),
    headers: ackHeaders,
    body: jsonEncode({
      'podNo'   : j.podNo,
      'day'     : day,
      'username': uLower, // server can also read x-driver header
    }),
  );
} catch (_) {}

// === Discord notify MUST succeed; otherwise, retry the job later ===
try {
  final uLower = (j.username ?? '').toLowerCase();
  final anyKey = 'pods/$day/$uLower/${j.podNo}_1.jpg';

  // Build headers based on auth path you used earlier
  final notifyHeaders = usedDeviceAuth
      ? await _deviceHeaders('application/json', anyKey)
      : _auth('application/json', anyKey);
  ...
  final notifyResp = await http.post(
    ...
    body: jsonEncode({
      'content':
          ...
          '\n🚚 $uLower\n📅 ${j.rddDate}',
      'imageUrls': urls, // the list you already built above
    }),
  );

  final ok = notifyResp.statusCode >= 200 && notifyResp.statusCode < 300;
  if (!ok) {
    debugPrint('WM[barcode_uploader]: notify failed '
        'status=${notifyResp.statusCode} body=${notifyResp.body}');
    // Mark as retryable: return false keeps the job for the next drain()
    return false;
  }
} catch (e) {
  // Network/DNS/timeout/etc → retry
  debugPrint('WM[barcode_uploader]: notify exception: $e');
  return false;
}
